...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            if self.config["triplet"]:
                return self.random_train_triplet_sample() #triplet sample, 3个样本
            else:
    ...
    def random_train_triplet_sample(self):
        standard_question_index = list(self.knwb.keys())
        #选两个正的，一个负的
        ps, ns = random.sample(standard_question_index, 2)
        if len(self.knwb[ps]) < 2:
            return self.random_train_triplet_sample()
        else:
            s1, s2 = random.sample(self.knwb[ps], 2)
            s3 = random.choice(self.knwb[ns])
            return [s1, s2, s3]
